...
@Repository
public class LandRepositoryImpl extends BasicRepositoryImpl<Land, ObjectId> implements LandRepository {
	...
	public String[] getSimpleFields(IContext<ObjectId> context) {
		return new String[] {
				Land.COLUMNNAME_NAME, Land.COLUMNNAME_ADDRESS, 
				Land.COLUMNNAME_LOCATION, 
				Land.COLUMNNAME_STATUS,
				Land.COLUMNNAME_DESCRIPTION };
	}
	...
	public boolean nameExists(IContext<ObjectId> context, String name, ObjectId excludeId) {
		...
		if (excludeId != null) {
			idCriteria = Criteria.where(Land.COLUMNNAME_ID).ne(excludeId);
		}

		Criteria criteria = CriteriaUtils.andOperator(idCriteria, Criteria.where(Land.COLUMNNAME_NAME).is(name.trim()));
	...
	public boolean delete(IContext<ObjectId> context, ObjectId id, ObjectId farmerId) {
		...
		Criteria criteria = CriteriaUtils.andOperator(
				Criteria.where(Land.COLUMNNAME_ID).is(id),
				Criteria.where(Land.COLUMNNAME_CONTACT_ID).is(farmerId)
			);
	...
	@Override
	public List<Land> getList(IContext<ObjectId> context, String text, String productName, String farmerName,
	...
	private List<CriteriaDefinition> getSearchCriteria(String text, String productName, String farmerName,
		...
		if (productIds != null && !productIds.isEmpty()) {
			productCriteria = Criteria.where(Land.COLUMNNAME_ID).in(productIds);
		}

		Criteria ignoreProductCriteria = null;
		if (ignoreProductIds != null && !ignoreProductIds.isEmpty()) {
			ignoreProductCriteria = Criteria.where(Land.COLUMNNAME_ID).nin(ignoreProductIds);
		}

		Criteria ownerCriteria = null;
		if (ownerIds != null && !ownerIds.isEmpty()) {
			ownerCriteria = Criteria.where(Land.COLUMNNAME_CONTACT_ID).in(ownerIds);
		}
		...
		if (categoryIds != null && !categoryIds.isEmpty()) {
			...
			for (ObjectId categoryId : categoryIds) {
				if (categoryId != null) {
					categoryCriterias.add(Criteria.where(Land.COLUMNNAME_CATEGORY_ID).all(categoryId));
				}
			}
		...
		if (countryIds != null && !countryIds.isEmpty()) {
			countryCriteria = Criteria.where(Land.COLUMNNAME_COUNTRY_ID).in(countryIds);
		}

		Criteria provinceCriteria = null;
		if (provinceIds != null && !provinceIds.isEmpty()) {
			provinceCriteria = Criteria.where(Land.COLUMNNAME_PROVINCE_ID).in(provinceIds);
		}

		Criteria districtCriteria = null;
		if (districtIds != null && !districtIds.isEmpty()) {
			districtCriteria = Criteria.where(Land.COLUMNNAME_DISTRICT_ID).in(districtIds);
		}

		Criteria locationCriteria = null;
		if (locationIds != null && !locationIds.isEmpty()) {
			locationCriteria = Criteria.where(Land.COLUMNNAME_LOCATION_ID).in(locationIds);
		}

		Criteria statusCriteria = null;
		if (statuses != null && !statuses.isEmpty()) {
			if (withAllOfOwnerIds == null || withAllOfOwnerIds.isEmpty()) {
				statusCriteria = Criteria.where(Land.COLUMNNAME_STATUS_VALUE).in(statuses);
			} else {
				statusCriteria = CriteriaUtils.orOperator(
					Criteria.where(Land.COLUMNNAME_STATUS_VALUE).in(statuses),
					Criteria.where(Land.COLUMNNAME_CREATED_BY_ID).in(withAllOfOwnerIds)
				);
			}
		}
		...
		if (withoutExpired) {
			...
			expiredCriteria = CriteriaUtils.andOperator(
					CriteriaUtils.orOperator(Criteria.where(Land.COLUMNNAME_CLOSED).exists(false),
							Criteria.where(Land.COLUMNNAME_CLOSED).is(null),
							Criteria.where(Land.COLUMNNAME_CLOSED).is(false)),
					CriteriaUtils.orOperator(Criteria.where(Land.COLUMNNAME_START_DATE).exists(false),
							Criteria.where(Land.COLUMNNAME_START_DATE).is(null),
							Criteria.where(Land.COLUMNNAME_START_DATE_VALUE).lte(currentDate)),
					CriteriaUtils.orOperator(Criteria.where(Land.COLUMNNAME_END_DATE).exists(false),
							Criteria.where(Land.COLUMNNAME_END_DATE).is(null),
							Criteria.where(Land.COLUMNNAME_END_DATE_VALUE).gte(currentDate)));
		}
