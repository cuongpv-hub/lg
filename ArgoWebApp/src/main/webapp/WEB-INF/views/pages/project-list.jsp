	...
	<div class="container">
		<div class="row">
			<h1 class="col-sm-12 text-center" style="font-weight: bold;">
				Danh sách dự án trên địa bàn
			</h1>
			<div class="col-md-12 col-sm-12 following">
				<form:form id="form-search" action="${pageContext.request.contextPath}/project/search" 
						...
						<div class="row">
							<div class="col-md-4 col-sm-6 col-xs-12">
								<form:input path="name" class="form-control"
										placeholder="Tên dự án, ..." id="searchName" name="searchName" />
							</div>
							<div class="col-md-3 col-sm-6 col-xs-12">
								<form:input path="company" class="form-control"
										placeholder="Tên doanh nghiệp, ..." id="searchCompany" name="searchCompany" />
							</div>
							<div class="col-md-5 col-sm-12 col-xs-12 sm-margin-top-10">
								<div class="input-group">
									<form:select path="categoryIds" id="categoryId" name="categoryId" 
										...
										title="Lựa chọn loại dự án" onchange="selectWithAllItem(this, '', true)">
										<option value="">Tất cả các loại dự án</option>
									...
									<span class="input-group-btn">
										...
										<c:if test="${ currentUser != null && currentUser.isCompany() }">
											<a class="btn btn-success" href="${pageContext.request.contextPath}/project/create" title="Thêm mới dự án">
						...
						<div class="row find-more-container" id="advanceSearchPanel" <c:if test="${ false && !searchDto.advanceSearch }">style="display: none;"</c:if>>
							<div class="col-md-3 col-sm-5 col-xs-12 padding-top-10">
								<form:select path="provinceId" id="searchProvince" name="searchProvince" 
									...
									title="Tỉnh/thành triển khai" onchange="selectProvince('searchProvince', 'searchDistrict', null, '', 'Tất cả', true)">
							...
							<div class="col-md-4 col-sm-7 col-xs-12 padding-top-10">
								<form:select path="districtIds" id="searchDistrict" name="searchDistrict" 
									class="form-control selectpicker" multiple="multiple" 
									title="Quận/huyện triển khai" onchange="selectWithAllItem(this, '', true)">
									<option value="">Tất cả</option>
								...
								<c:choose>
	 								<c:when test="${ dto.mainImage != null && dto.mainImage != '' }">
										<img src="${pageContext.request.contextPath}/upload/project/${dto.mainImage}" 
											alt="${ dto.name }" class="img-fluid rounded-circle with-error-src"
				...
				<ul class="pagination">
					<c:choose>
						...
						<c:otherwise>
							<li class="paging-item">
								<a href="${pageContext.request.contextPath}/project/page?page=1">&laquo;</a>
							</li>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach var="page" begin="1" end="${pageCount}">
						<c:choose>
							...
							<c:otherwise>
								<li class="paging-item">
									<a href="${pageContext.request.contextPath}/project/page?page=${ page }">${ page }</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="paging-item">
								<a href="${pageContext.request.contextPath}/project/page?page=${ pageCount }">&raquo;</a>
							</li>
						</c:otherwise>
					</c:choose>
				</ul>
