	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-12 col-sm-12 following">
				<form:form id="form-search" action="${pageContext.request.contextPath}/product/search" 
						...
						<div class="row">
							<div class="col-md-4 col-sm-6 col-xs-12">
								<form:input path="name" class="form-control"
										placeholder="Tên nông sản, ..." id="searchName" name="searchName" />
							</div>
							...
							<div class="col-md-5 col-sm-12 col-xs-12 sm-margin-top-10">
								<div class="input-group">
									<form:select path="categoryIds" id="categoryId" name="categoryId" 
										...
										title="Loại nông sản" onchange="selectWithAllItem(this, '', true)">
									...
									<span class="input-group-btn">
										...
										<c:if test="${ currentUser != null && currentUser.isFarmer() }">
											<a class="btn btn-success" href="${pageContext.request.contextPath}/product/create" title="Thêm mới nông sản">
						...
						<div>
							<a href="${pageContext.request.contextPath}/product/${ dto.id }/view" title="${ dto.name }">
								<c:if test="${ dto.mainImage != null && dto.mainImage != '' }">
									<img src="${pageContext.request.contextPath}/upload/product/${dto.mainImage}" 
								...
								<div class="wid-u-info">
									...
									<c:if test="${ dto.moneyFrom != null || dto.moneyTo != null }">
										<p class="text-muted m-b-5 font-13" title="Giá bán giao động theo thị trường">
											<i aria-hidden="true" class="fa fa-money"></i>
											<span class="text-number">${ dto.moneyFrom }</span> - <span class="text-number">${ dto.moneyTo }</span>
							...
							<c:if test="${ dto.owner || dto.canApprove }">
								...
								<div class="row">
									<div class="col-sm-6 col-xs-12">
										<c:if test="${ dto.owner }">
											<a class="btn btn-success" href="${pageContext.request.contextPath}/product/${ dto.id }/edit">Chỉnh sửa</a>
										</c:if>
									</div>
									<div class="col-sm-6 col-xs-12">
										<c:if test="${ dto.canApprove }">
											<a class="btn btn-primary pull-right" href="${pageContext.request.contextPath}/product/${ dto.id }/view">Xem và phê duyệt</a>
										</c:if>
									</div>
								</div>
							</c:if>
				...
				<ul class="pagination">
					<c:choose>
						...
						<c:otherwise>
							<li class="paging-item">
								<a href="${pageContext.request.contextPath}/product/page?page=1">&laquo;</a>
							</li>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach var="page" begin="1" end="${pageCount}">
						<c:choose>
							...
							<c:otherwise>
								<li class="paging-item">
									<a href="${pageContext.request.contextPath}/product/page?page=${ page }">${ page }</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="paging-item">
								<a href="${pageContext.request.contextPath}/product/page?page=${ pageCount }">&raquo;</a>
							</li>
						</c:otherwise>
					</c:choose>
				</ul>
