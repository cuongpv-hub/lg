...
<c:if test="${ dto.posterImage != null && dto.posterImage != '' }">
	<style type="text/css">
		.profile-bg-picture {
			background-image: url('${pageContext.request.contextPath}/upload/land/${ dto.posterImage }') !important;
					...
					<c:if test="${ currentUser != null }">
						<c:if test="${ dto.owner && dto.status.status != 1 }">
							<div class="row padding-top-10">
								<div class="col-xs-12">
									<a class="btn btn-primary" href="${pageContext.request.contextPath}/land/${ dto.id }/edit">
                        ...
                        <div class="offset-md-1 offset-sm-1 col-md-4 col-sm-4 col-xs-12 tab-sidebar text-inline">
                            <div class="mobile-box">
                                <div class="box-summary link-list">
									...
									<c:if test="${ dto.volume != null }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												...
												<span class="content-label">&nbsp;<strong>Số lượng lô, mảnh, thửa đất:</strong></span>
												<span class="content">
									...
									<c:if test="${ dto.square != null }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												...
												<span class="content-label">&nbsp;<strong>Tổng diện tích:</strong></span>
												<span class="content">
									...
									<c:if test="${ dto.tree != null && dto.tree != '' }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												<i aria-hidden="true" class="fa fa-tree"></i>
												<span class="content-label">&nbsp;<strong>Đất hiện đang trồng:</strong></span>
												<span class="content">${ dto.tree }</span>
											</div>
										</div>
									</c:if>
									
									<c:if test="${ dto.animal != null && dto.animal != '' }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												<i aria-hidden="true" class="fa fa-globe"></i>
												<span class="content-label">&nbsp;<strong>Chăn nuôi:</strong></span>
												<span class="content">${ dto.animal }</span>
											</div>
										</div>
									</c:if>
									
									<c:if test="${ dto.forest != null && dto.forest != '' }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												<i aria-hidden="true" class="fa fa-tree"></i>
												<span class="content-label">&nbsp;<strong>Đất đang trồng rừng:</strong></span>
												<span class="content">${ dto.forest }</span>
											</div>
										</div>
									</c:if>
									...
									<div class="row summary-item">
										<div class="col-xs-12 summary-content" title="Loại đất">
											<i aria-hidden="true" class="fa fa-tags"></i>
											<!-- <span class="content-label">&nbsp;<strong>Loại đất:</strong></span> -->
											<span class="content">
												<c:forEach items="${dto.categories}" var="category" varStatus="loop">
													<a class="tag-item small" href="${pageContext.request.contextPath}/land?categoryId=${ category.id }" title="${ category.name }">
														${ category.name }
													</a>
												</c:forEach>
											</span>
									...
									<c:if test="${ dto.nears != null && !dto.nears.isEmpty() }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
												<i aria-hidden="true" class="fa fa-truck"></i>
												<span class="content-label">&nbsp;<strong>Vị trí:</strong></span>
												<span class="content">
													<c:forEach items="${dto.nears}" var="category" varStatus="loop">
									...
									<c:if test="${ dto.formCooperations != null && !dto.formCooperations.isEmpty() }">
										<div class="row summary-item">
											<div class="col-xs-12 summary-content">
                    ...
                    <div class="row content-panal-body card-block following table-row">
                        <div class="col-md-12 farmer-register">
                            <h4 class="color-1ab394 text-bold margin-top-15 margin-bottom-10" style="text-transform: uppercase;">
                            	Danh sách các dự án tham gia sử dụng&nbsp;&nbsp;&nbsp;&nbsp;
                            	...
                            	<c:choose>
									...
									<c:when test="${ currentUser != null && currentUser.isCompany() }">
										<a id="addProject" href="javascript:;" class="btn btn-sm btn-success" 
											title="Đăng ký sử dụng thửa đất" onclick="openProjectModal()">
											<i class="fa fa-handshake-o"></i> Đăng ký sử dụng thửa đất
										</a>
									</c:when>
									<c:when test="${ currentUser == null }">
										<a id="addProject" href="javascript:;" class="btn btn-sm btn-success" 
											title="Đăng ký sử dụng thửa đất" onclick="loginAndLinkProjectModal()">
											<i class="fa fa-handshake-o"></i> Đăng ký sử dụng thửa đất
										</a>
									</c:when>
								</c:choose>
                        ...
                        <c:choose>
		 					<c:when test="${ dto.projectDtos == null || dto.projectDtos.isEmpty() }">
		 						<div class="col-md-12">
		 							<div class="text-center">Chưa có cá nhân hoặc tổ chức đăng ký tham gia sử dụng</div>
		 						</div>
		 					</c:when>
		 					<c:otherwise>
														...
														<c:when test="${ project.owner }">
															<c:if test="${ project.status == null || project.status.status == 0 }">
																<button type="button" class="btn btn-success btn-block" onclick="approveLink('${ project.linkId }', 0, 1)">
																	<i class="fa fa-check"></i> Xác nhận liên kết sử dụng
																</button>
															</c:if>
															
															<button type="button" class="btn btn-danger btn-block" onclick="removeLink('${ project.linkId }', 0, 1)">
																<i class="fa fa-times"></i> Từ chối liên kết sử dụng
															</button>
														</c:when>
													</c:choose>
												</c:if>
										...
										<div class="blog-relative blog-hover blog-hover-scale" style="height: 100%">
											<div class="blog-image blog-vgradient-black-lg" style="height: 100%">
												<div class="overlay" style="height: 100%">
													<img src="${pageContext.request.contextPath}/upload/land/${ image }" 
